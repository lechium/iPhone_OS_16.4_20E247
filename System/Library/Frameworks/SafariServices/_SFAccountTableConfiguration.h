//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

__attribute__((visibility("hidden")))
@interface _SFAccountTableConfiguration : NSObject
{
    _Bool _shouldShowIcons;	// 8 = 0x8
    _Bool _shouldShowSearchBar;	// 9 = 0x9
    _Bool _supportsDelete;	// 10 = 0xa
    _Bool _supportsShare;	// 11 = 0xb
    _Bool _shouldConfigureMultipleSelection;	// 12 = 0xc
}

@property(nonatomic) _Bool shouldConfigureMultipleSelection; // @synthesize shouldConfigureMultipleSelection=_shouldConfigureMultipleSelection;
@property(nonatomic) _Bool supportsShare; // @synthesize supportsShare=_supportsShare;
@property(nonatomic) _Bool supportsDelete; // @synthesize supportsDelete=_supportsDelete;
@property(nonatomic) _Bool shouldShowSearchBar; // @synthesize shouldShowSearchBar=_shouldShowSearchBar;
@property(nonatomic) _Bool shouldShowIcons; // @synthesize shouldShowIcons=_shouldShowIcons;

@end

