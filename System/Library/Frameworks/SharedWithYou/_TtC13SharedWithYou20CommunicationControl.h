//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <UIKit/UIControl.h>

@class MISSING_TYPE, UILabel;

__attribute__((visibility("hidden")))
@interface _TtC13SharedWithYou20CommunicationControl : UIControl
{
    MISSING_TYPE *touchUpInsideActionIdentifier;	// 8 = 0x8
    MISSING_TYPE *communicationChannel;	// 16 = 0x10
    MISSING_TYPE *contentView;	// 24 = 0x18
    MISSING_TYPE *imageView;	// 32 = 0x20
    MISSING_TYPE *label;	// 40 = 0x28
    MISSING_TYPE *sBehaviors;	// 0 = 0x0
    MISSING_TYPE *menu;	// 48 = 0x30
    MISSING_TYPE *enablementLock;	// 56 = 0x38
}

- (void).cxx_destruct;	// IMP=0x000000000002daf0
- (id)initWithFrame:(struct CGRect)arg1;	// IMP=0x000000000002da90
- (id)contextMenuInteraction:(id)arg1 configurationForMenuAtLocation:(struct CGPoint)arg2;	// IMP=0x000000000002da30
- (void)traitCollectionDidChange:(id)arg1;	// IMP=0x000000000002d980
- (void)tintColorDidChange;	// IMP=0x000000000002d7f0
- (id)initWithCoder:(id)arg1;	// IMP=0x000000000002d680
@property(nonatomic) _Bool enabled;
- (_Bool)isEnabled;	// IMP=0x000000000002c450
@property(nonatomic, readonly) UILabel *label; // @synthesize label;
@property(nonatomic, readonly) struct CGSize intrinsicContentSize;

@end

