//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class CRLColor, CRLStrokePattern;

@interface CRLMutableStroke
{
}

+ (id)emptyStroke;	// IMP=0x00200000003b0902
+ (id)stroke;	// IMP=0x00100000003b08cc
- (id)copyWithZone:(struct _NSZone *)arg1;	// IMP=0x00200000003b0938
- (void)setPatternPropertiesFromStroke:(id)arg1;	// IMP=0x00100000003b08ba
- (void)setPropertiesFromStroke:(id)arg1;	// IMP=0x00100000003b08a8
@property(copy, nonatomic) CRLStrokePattern *pattern;
@property(nonatomic) unsigned long long join;
@property(nonatomic) double miterLimit;
@property(nonatomic) unsigned long long cap;
@property(nonatomic) double actualWidth;
@property(nonatomic) double width;
@property(copy, nonatomic) CRLColor *color;

@end

